/* =========================================
   ELIF'S PHONICS HUB — elif-script.js
   Week navigation + recorder (all 10 days)
   ========================================= */
...
const WEEK_TITLES = {
  1: "Week 1 · Short Vowels · a · e · i · o · u",
  2: "Week 2 · Sight Words & Pronouns"
};
...
WEEKS.forEach(week => {
  DAYS.forEach(day => {
    if (localStorage.getItem(`elif-submitted-w${week}-${day}`) === "true") {
      markDone(week, day);
    }
  });
});

// ── Set up recorders for all 10 days ─────────────────────────────
...
function setupRecorder(week, day) {
  ...
  if (!recordBtn) return; // safety guard
  ...
  submitBtn.addEventListener("click", async () => {
    ...
    const filename = `Elif_W${week}_${capitalise(day)}_${date}.${ext}`;
    try {
      const base64 = await blobToBase64(audioBlob);
      await fetch(DRIVE_UPLOAD_URL, {
        method: "POST", mode: "no-cors",
        body: JSON.stringify({ filename, mimeType: audioBlob.type, data: base64,
                               student: "Elif", week, day, date })
      });
      statusMsg.className = "status-msg success";
      statusMsg.textContent = "✅ Sent! Great work today, Elif! 🌟";
      localStorage.setItem(`elif-submitted-w${week}-${day}`, "true");
      markDone(week, day);
    } catch {
      statusMsg.className = "status-msg error";
...
function refreshTabDoneStates(week) {
  ...
  DAYS.forEach(day => {
    ...
    if (!tab) return;
    tab.classList.toggle("done", localStorage.getItem(`elif-submitted-w${week}-${day}`) === "true");
  });
}
