...
function openWeekView(week) {
  ...
  // Update title
  const titles = {
    1: "Week 1 · Home & Hobbies",
    2: "Week 2 · Chapter 1 · A Day in England",
    3: "Week 3 · Chapter 2 · The Magical House",
    4: "Week 4 · Chapter 3 · The Ancient Ships of Phoenicia"
  };
...
WEEKS.forEach(week => {
  DAYS.forEach(day => {
    const key = `submitted-w${week}-${day}`;
...
function setupRecorder(week, day) {
  ...
  if (!recordBtn) return; // guard if element missing
  ...
  // ── Submit button: upload to Google Drive ─────────────────────
  submitBtn.addEventListener("click", async () => {
    ...
    const filename = `Bilge_W${week}_${capitalise(day)}_${date}.${ext}`;
    ...
    try {
      ...
      statusMsg.textContent = "✅ Sent! Great work today, Bilge! 💜";
      localStorage.setItem(`submitted-w${week}-${day}`, "true");
      markDone(week, day);
...
function markDone(week, day) {
  ...
  // Put a ✓ dot on the tab (only visible when that week is open)
  const tab = document.querySelector(`.tab[data-day="${day}"]`);
  if (tab) tab.dataset[`donew${week}`] = "true";

  // Refresh tab done state if this is the current week
...
function refreshTabDoneStates() {
  DAYS.forEach(day => {
    const tab = document.querySelector(`.tab[data-day="${day}"]`);
    if (!tab) return;
    const key = `submitted-w${currentWeek}-${day}`;
    if (localStorage.getItem(key) === "true") {
