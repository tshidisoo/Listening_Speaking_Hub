/* =========================================
   ARAS'S ABC ADVENTURE — aras-script.js
...
DAYS.forEach(day => {
  if (localStorage.getItem(`aras-submitted-${day}`) === "true") {
    markDone(day);
  }
});
...
function setupRecorder(day) {
  ...
  submitBtn.addEventListener("click", async () => {
    ...
    const filename = `Aras_${capitalise(day)}_${date}.${ext}`;
    try {
      const base64 = await blobToBase64(audioBlob);
      await fetch(DRIVE_UPLOAD_URL, {
        method: "POST", mode: "no-cors",
        body: JSON.stringify({ filename, mimeType: audioBlob.type, data: base64,
                               student: "Aras", day, date })
      });
      statusMsg.className = "status-msg success";
      statusMsg.textContent = "✅ Sent! Amazing singing, Aras! 🎯";
      localStorage.setItem(`aras-submitted-${day}`, "true");
      markDone(day);
    } catch {
      statusMsg.className = "status-msg error";
...
function refreshTabDoneStates() {
  DAYS.forEach(day => {
    ...
    if (!tab) return;
    tab.classList.toggle("done", localStorage.getItem(`aras-submitted-${day}`) === "true");
  });
}
